...
Некоторые глобальные объекты, их методы и свойства.

## `Персонаж (character)`

Персонаж создан за вас, но передаётся в большинство функций, которые вы описываете.

Он состоит из объекта без методов, у которого есть два свойства:

#### `качества (qualities)`

Объект qualities сопоставляет идентификаторы качеств с их текущими значениями.

Ваш код находит значения качеств, читая этот объект, например:

    var gold = character.qualities.gold;

У вас есть два способа установки качеств. Если вы знаете, что качество, которое вы задаёте, не будет отображаться в интерфейсе пользователя, то вы можете задать его напрямую:

    character.qualities.gold += 1;

Если оно отображается на экране, то этот подход не обновит панель персонажа, и игрок будет видеть старое значение. Вместо этого лучше использовать метод объекта `System` под названием `setQuality`, который также следит за обновлением интерфейса:

    system.setQuality('gold', character.qualities.gold+1);

Второй подход также можно использовать для скрытых качеств, поэтому его универсальность делает его рекомендованным способом.

#### `песочница (sandbox)`

Не все данные, которые вы хотите ассоциировать с персонажем, красиво подходят под качества. Песочница -- это общее место склада любых данных, которые вы хотите хранить в процессе игры, например:

    character.sandbox.roomsSearched.push('bathroom');

Данные песочницы никогда не выводятся пользователю, так что вы можете использовать в ней любые структуры данных Javascript без ограничений.

## `Система (System)`

Система представляет интерфейс между вашим кодом и пользователем.
Вы не создаёте своего объекта `System`, он передаётся в ваш код.
